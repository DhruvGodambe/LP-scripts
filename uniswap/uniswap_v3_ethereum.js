...
const GRAPHQL_ENDPOINT_ETHEREUM = "https://api.thegraph.com/subgraphs/name/uniswap/uniswap-v3";
const GRAPHQL_ENDPOINT_OPTIMISM = 'https://api.thegraph.com/subgraphs/name/messari/uniswap-v3-optimism';
...
async function getLiquidityPools() {
  ...
  let skip = 0;
  let allPools = [];
  const ethereum_pools_max_page = 5000;
  ...
  while (true) {
    try {
      ...
      const data = await request(GRAPHQL_ENDPOINT_ETHEREUM, query, { first: batchSize, skip });
...
async function saveToExcel(liquidityPools) {
  ...
  // Save workbook
  await workbook.xlsx.writeFile('uniswap_v3_ethereum.xlsx');
}
