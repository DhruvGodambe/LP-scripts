...
const GRAPHQL_ENDPOINT_ETHEREUM = "https://api.thegraph.com/subgraphs/name/sushiswap/exchange";
...
async function getLiquidityPools() {
  ...
  let skip = 0;
  let allPools = [];
  const ethereum_pools_max_page = 4000;
...
async function saveToExcel(liquidityPools) {
  ...
  // Save workbook
  await workbook.xlsx.writeFile('sushiswap_v2_ethereum.xlsx');
}
